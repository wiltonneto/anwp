...
global $ph_version;
$ph_version = '6.0.5';  //latest release
...
function pluginhunt_admin_menu() {
    ...
    if(function_exists('epic_envato_api_activation')){
    $pluginhunt_integrations  = add_submenu_page( $pluginhunt_slugs['home'], 'Integrations', 'Integrations', 'manage_options', $envato_auto_affiliate_slugs['settings'] , 'epic_integrations_menu' );   
    add_action( 'admin_print_styles-' . $pluginhunt_integrations, 'envato_auto_affiliate_admin_cssloader' );
    add_action( 'admin_print_scripts-' . $pluginhunt_integrations, 'envato_auto_affiliate_admin_jsloader' ); 
    }
    $pluginhunt_tools = add_submenu_page( $pluginhunt_slugs['home'], 'Add Ons', 'Add Ons', 'manage_options', $pluginhunt_slugs['addons'] , 'pluginhunt_addons' );
    ...
    <div class='row ph-addon-list'>
      <h3><?php _e('Plugin Hunt Theme Official Add ons'); ?></h3>
        <div class='ph-addon'>
            ...
            <div class='ph-des'>
            <div class='ph-title'><?php _e('Popular This - $9.99', 'pluginhunt'); ?></div>
...
function pluginhunt_home(){
  ...
  <div class='main'>
      ...
      <?php $docs = "http://epicplugins.com/help/doc/plugin-hunt-theme/"; ?>
      <br/>
      <p class='button-hunt'>
...
function ph_mailchimpsubscribe(){
	...
	//$is_valid_nonce = ( isset( $_POST[ 'checksum' ] ) && wp_verify_nonce( $_POST[ 'checksum' ], basename( __FILE__ ) ) ) ? 'true' : 'false';
	
	//if($is_valid_nonce == 'false'){
	//	$response['msg'] = "Unable to subscribe user. nonce failed";
	//	echo json_encode($response);
	//	die();	
	//}
		
	//Before we go any further let's make sure that their PHP version is high enough 
	//to run mailchimp. 
...
function UserSubscribed(){
	write_log('In User Subscribed function');
	$uid = get_current_user_id();
	...
	if($uid == 0 && isset($_COOKIE['ph_mailchimp_subscribed'])){
		//$uid is not set and there is a cookie. 
		$subscribed =  $_COOKIE['ph_mailchimp_subscribed'];
		write_log('Cookie is set: ' . subscribed);
	} elseif($uid > 0){
		//$uid is set so let's check their user meta
		$subscribed = get_user_meta($uid,'ph_mailchimp_subscribed',true);
		write_log('meta data: ' . subscribed);
	}
...
function ph_mailchimp($action){
  ...
  <div class="newsletter-box well">
      ...
      <div style="display:none"><input name="utf8" type="hidden" value=""></div>
        <input class="inputfield" id="mce-EMAIL" name="EMAIL" placeholder="'. __('Seu email','phtheme') .'" type="email">
        <input name="subscribe" class="subscribeButton" id="subscribeButton" type="submit" value="'. __('Ok','phtheme') .'">
...
function ph_grid_theme_register_required_plugins() {
    ...
    $plugins = array(
        ...
        array(
            'name'               => 'User Photo Mod', // The plugin name.
            'slug'               => 'user-photo-mod', // The plugin slug (typically the folder name).
            'source'             => get_stylesheet_directory() . '/plugins/user-photo-mod.zip', // The plugin source.
...
if ( ! class_exists( 'Plugin_Hunt_License_Menu' ) ) {
     ...
     */
    Plugin_Hunt_License_Menu::instance( __FILE__, 'Plugin Hunt Theme', '6.0.5', 'theme', 'https://epicplugins.com/' );
}
