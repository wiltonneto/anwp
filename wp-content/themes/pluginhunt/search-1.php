...
<?php
	echo __('Products for ','ph_theme') . '<span class="search-query">'. $s . '</span><span class="nr"> ' . $NumResults . ' ' . $r . '</span></h1>';  
	...
	if ( have_posts() ) : ?>
			...
			<?php if ( has_post_thumbnail() ) { //#WHFIX 24/03/2015: added data-attr to end  ?>
						<div class = 'reddit-post<?php echo $mob; ?> pull-left' id='reddit-post-<?php echo $post->ID;?>' data-ph-url="<?php  echo post_permalink( $post->ID ); ?>" data-slug='<?php echo $post->post_name; ?>' data-id='<?php echo $post->ID; ?>' data-url = '<?php echo $url; ?>' data-auth = '<?php echo $auth; ?>' data-rajax = '<?php echo $post->ID; ?>' data-rups = '<?php echo $postvote;?>' data-pname='<?php echo $pname;?>' data-profurl="<?php echo $profileUrl; ?>" data-red-current = <?php echo $al;?> data-red-like = "<?php echo $redclassd; ?>">
